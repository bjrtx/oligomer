...
CHAIN_FILENAME = "Bfr_molmap_chain?_res5.mrc"
...
if __name__ == "__main__":
    ...
    example_file = CHAIN_FILENAME.replace("?", "A")

    # dimers = ["aq", "bo", "cv", "du", "ep", "fr", "gk", "hn", "is", "jt", "lw", "mx"]
    for ab in hotspots.dimer_names:
        logging.info(f"Writing maps for the chains in {ab}.")
        chains = [
            hotspots.read_mrc(CHAIN_FILENAME.replace("?", char.upper())) for char in ab
        ]
        # There should be exactly one nonzero label in the intersection of chains[0] and the
        # heme.
        ...
        for char, chain in zip(ab, aug_chains):
            # Copy an existing file to preserve the header. MRC headers have many
            # parameters to set. (In fact, Chimera seems to omit some of them.)
